...
class BiPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if self.is_vision_model:
            ...
            self.tokenizer = tokenizer  # Old transformers
            # self.processor = tokenizer # New transformers
        ...
        if model_name is None:
            self.vec_dir = f"./vector/{self.behavior}/layer{self.layer}"
        else:
            self.vec_dir = f"./vector/{self.behavior}_{model_name}/layer{self.layer}"
        ...
        with PartialState().local_main_process_first():
            ...
            if eval_dataset is not None:
                if isinstance(eval_dataset, dict):
                    eval_dataset = {
                        "test_dataset_add": eval_dataset["test_dataset_add"].map(
                            self.tokenize_row,
                            num_proc=self.dataset_num_proc,
                            writer_batch_size=10,
                        ),
                        "test_dataset_sub": eval_dataset["test_dataset_sub"].map(
                            self.tokenize_row,
                            num_proc=self.dataset_num_proc,
                            writer_batch_size=10,
                        ),
    ...
    def get_batch_loss_metrics(
        ...
        if train_eval == "train":
            ...
            model.module.model.layers[self.layer].set_multiplier(multiplier)
        else:
        ...
        if isinstance(model, torch.nn.parallel.DistributedDataParallel):
            metrics[f"{prefix}multiplier"] = model.module.model.layers[
                self.layer
            ].multiplier
        else:
    ...
    def evaluation_loop(
        ...
        """
        Custom evaluation_loop that also saves the full (unsharded) .vec parameter on rank 0.
        """
        from torch.distributed.fsdp import FullStateDictConfig, StateDictType
        ...
        # Optionally, sample from the model on the main process
        if self.generate_during_eval:
            if self.accelerator.is_main_process:
                ...
                prompts = random_batch["prompt"]
                for i, (prompt, policy, ref) in enumerate(
                    zip(prompts, policy_output_decoded, ref_output_decoded)
                ):
                    policy_after_prompt = policy[len(prompt) :]
                    ref_after_prompt = ref[len(prompt) :]
                    print("-" * 60)
                    print(f"Random sample {i} of {len(prompts)}")
                    print("Prompt:")
                    print(prompt)
                    print("Policy output (beyond prompt):")
                    print(policy_after_prompt)
                    print("Reference output (beyond prompt):")
                    print(ref_after_prompt)
                    print("-" * 60)
            self.accelerator.wait_for_everyone()

        # ---------------------------------------------------------
        # Run the normal Trainer evaluation loop to compute metrics
        # ---------------------------------------------------------
        ...
        # SAVE metrics
        if self.accelerator.is_main_process:
            metrics_file = f"{self.vec_dir}/evaluation_metrics.jsonl"
            # Get means of stored metrics
            stored_metrics = {}
            if "eval" in self._stored_metrics:
                for key, values in self._stored_metrics["eval"].items():
                    if values:  # Check if we have any values
                        if isinstance(values[0], torch.Tensor):
                            mean_value = torch.stack(values).mean().item()
                        else:
                            mean_value = np.mean(values)
                        stored_metrics[key] = mean_value

            # Combine with initial_output metrics
            metrics = {**initial_output.metrics, **stored_metrics}
            ...
            with open(metrics_file, "a") as f:
                ...
                print(f"[evaluation_loop] Saved metrics to {metrics_file}")

        # -----------------------------------------------
        # Now gather & save your .vec param
        # -----------------------------------------------
        current_multiplier = self.model.model.layers[self.layer].multiplier
        if current_multiplier > 0:
            self.epoch_for_saving_vec += 1

            steer_vec = self.model.model.layers[self.layer].vec.detach().cpu()
            gpu_rank = self.accelerator.device.index
            # Check is steer_vec is torch.Size([0]):
            if steer_vec.numel() != 0:
                print(f"[evaluation_loop] steer_vec shape = {steer_vec.shape}")
                print("[evaluation_loop] First 10 values:", steer_vec[:10])
                # Get GPU rank
                filename = f"{self.vec_dir}/vec_ep{int(self.epoch_for_saving_vec)}_layer{self.layer}_gpu{gpu_rank}.pt"
                torch.save(steer_vec, filename)
                print(
                    f"[evaluation_loop] Saved full steer_vec from {gpu_rank} to {filename}"
                )
            else:
                print(
                    f"[evaluation_loop] steer_vec for {gpu_rank} is empty. Skip saving."
                )
                   # ---------------------------------------------------------
                    # 4) ALSO Save a partial checkpoint (optimizer, LR schedule, seeds)
                    # ---------------------------------------------------------
            if self.accelerator.is_main_process:
                import random
                partial_ckpt_path = (
                    f"{self.vec_dir}/partial_ckpt_epoch{int(self.epoch_for_saving_vec)}.pt"
                )

                # Build dictionary for partial checkpoint
                partial_ckpt = {
                    "epoch": self.state.epoch,
                    "global_step": self.state.global_step,
                    # "epoch_for_saving_vec" could also be stored if you want
                    "optimizer_state": self.optimizer.state_dict(),
                    # If your trainer doesn't have self.lr_scheduler, adapt accordingly
                    "scheduler_state": (
                        self.lr_scheduler.state_dict() if hasattr(self, "lr_scheduler") and self.lr_scheduler else None
                    ),
                    "rng_states": {
                ...
                if torch.cuda.is_available():
                    partial_ckpt["rng_states"]["cuda"] = torch.cuda.get_rng_state_all()

                torch.save(partial_ckpt, partial_ckpt_path)
                print(f"[evaluation_loop] Saved partial checkpoint to {partial_ckpt_path}")


            # # find and print this param: _fsdp_wrapped_module.model.layers.20.vec

            # # unwrap to get the raw FSDP model
            # fsdp_wrapped_model = self.accelerator.unwrap_model(self.model)

            # # Use the FSDP.state_dict_type context
            # with FSDP.state_dict_type(
            #     fsdp_wrapped_model,
            #     StateDictType.FULL_STATE_DICT,
            #     FullStateDictConfig(offload_to_cpu=True, rank0_only=True),
            # ):
            #     # On rank 0, full_sd is unsharded. On other ranks, it's empty/None.
            #     full_sd = fsdp_wrapped_model.state_dict()
            #     for name, param in full_sd.items():
            #         print(name, param.shape)

            # if self.accelerator.is_main_process:
            #     # You can look at full_sd.keys() if you're not sure of the exact name
            #     param_key = f"model.layers.{self.layer}.vec"
            #     steer_vec = full_sd[param_key]  # unsharded param on rank 0
            #     print(f"[evaluation_loop] steer_vec shape = {steer_vec.shape}")
            #     print("[evaluation_loop] First 10 values:", steer_vec[:10])
            #     filename = f"{self.vec_dir}/vec_ep{self.epoch_for_saving_vec}_layer{self.layer}.pt"
            #     torch.save(steer_vec, filename)
            #     print(f"[evaluation_loop] Saved full steer_vec to {filename}")
